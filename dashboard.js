...
function processSheet(rows) {
  ...
  // Filtrar solo meses con datos reales (Ingresos por Ventas != 0) — punto 2 del pedido
  ...
  // Reconstruir dataRows alineado al monthCols ya filtrado
  dataRows = {};
  ...
  if (selectedIdx === null || selectedIdx >= monthCols.length) selectedIdx = monthCols.length - 1;
  // limpiar selección de rango si quedó fuera de rango tras un refresh
  rangeSel = new Set(Array.from(rangeSel).filter(i => i < monthCols.length));
  ...
  document.getElementById('row-count').textContent = `${monthCols.length} ${monthCols.length===1?'período':'períodos'} con datos cargados · USD`;
}
...
function renderCharts() {
  ...
  charts.c6 = new Chart(document.getElementById('c6'), {
    ...
    options: { responsive:true, maintainAspectRatio:false,
      ...
      scales: { x:{grid:{display:false}, ticks:{autoSkip:false, maxRotation:35, font:{size:10}, color:textColor}}, y:{grid:{color:gridColor}, ticks:{color:textColor, callback:v=>'$'+(v/1000).toFixed(0)+'K'}} }
    }
  });
  ...
  charts.c7 = new Chart(document.getElementById('c7'), {
    ...
    options: { responsive:true, maintainAspectRatio:false,
      ...
      scales: { x:{grid:{display:false}, ticks:{color:textColor}}, y:{grid:{color:gridColor}, ticks:{color:textColor, callback:v=>'$'+(v/1000).toFixed(0)+'K'}} }
    }
  });
  ...
  if (finMonthCols.length > 0) {
    ...
    const patrimonioSerie = finMonthCols.map((c,i) =>
      getFinVal('TOTAL', i) + getFinVal('Creditos por Ventas', i) + getFinVal('Otros Creditos', i) + getFinVal('Bienes de Uso', i)
    );
    destroyChart('c5');
    charts.c5 = new Chart(document.getElementById('c5'), {
      type: 'bar',
      data: { labels: finLabels, datasets: [
        { label:'Total caja', data: cajaSerie, backgroundColor:'rgba(139,127,214,.75)', borderRadius:3 },
        { label:'Patrimonio total', data: patrimonioSerie, type:'line', borderColor: COLORS.orange, borderWidth:2.5, pointRadius:4, fill:false, tension:.3 },
      ]},
      options: { responsive:true, maintainAspectRatio:false,
        plugins: { legend:{display:false}, tooltip:{ callbacks:{ label: c => ` ${c.dataset.label}: $${c.parsed.y.toLocaleString()}` } } },
        scales: {
